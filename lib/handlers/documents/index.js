  ...
  async.waterfall([
    ...
    function remapDocuments(documentsRes, cb) {
      ...
      res.send(200, documentsRes.body.data.map(function mapDocuments(document) {
        return {
          documentId: /realtime/.test(document.document_url) ?
            document.document_url.replace("/documents/", "") :
            document.id,
          typeId: document.document_type.id,
          providerId: document.provider ? (document.provider.client ? document.provider.client.id : "") : "",
